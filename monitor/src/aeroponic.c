...
set_processes(int processes[], char* str, int uptime)
{
	...
	for (int i = 0; i < MONITOR__PROCESS__MAX && proc != NULL; i++) {
	    ...
	    if (processes[i] == atoi(proc)) {
		log_info(get_log_file(), "Process %s failed after %d uptime", get_process_string(i), uptime);
	    }
...
main()
{
	...
	process_start_time = time(NULL);
	    ...
	    get_log_file(), "Failed to start monitor child process");

int x = 0;
        for ( ; ; ) {
	    check(proc_failures < PROC_FAILURE_LIMIT, get_log_file(), "Processes failing, need maintenance");
		...
		if (errno == EAGAIN) { /* pipe empty */
			...
			log_err(get_log_file(), "Processes inactive, need aeroponic maintenance");
		    }
		    sleep(5);
		    continue;
		} else if (errno == EINVAL) {
		    log_err(get_log_file(), "Too much data being sent");
		} else {
		    log_err(get_log_file(), "read error");
		}
		break;
	    case 0:
		break;
		...
		if (strncmp(process_buff, "ERROR:", strlen("ERROR:")) != 0) { /* solenoid or pump failed, updating processes array */
		    proc_failures++;
		    check(set_processes(processes, process_buff, (time(NULL) - process_start_time)) == 0,
			get_log_file(), "Failed to set processes array");
		    initial_setup = false;
		    x++;
		} else { /* Monitor produced error Resetting processes */
		    debug("else");
		    proc_failures++;
		    check(kill_processes(processes) == 0, get_log_file(), "Error killing child processes");
		    check((processes[MONITOR] = start_monitor_child(pipes)) != -1,
			get_log_file(), "Failed to start monitor child process");
		    initial_setup = true;
		}
